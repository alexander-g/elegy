...
class UNetTest(TestCase):
    def test_basic_predict(self):
        ...
        y = model.predict(x, initialize=True)
        ...
        open(pklpath, "wb").write(pickle.dumps(model.module.get_default_parameters()))

        new_ur18 = elegy.nets.unet.UNet_R18(weights=pklpath)
        y2 = elegy.Model(new_ur18, run_eagerly=True).predict(x, initialize=True)
